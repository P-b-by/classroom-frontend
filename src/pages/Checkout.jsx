...
// Allow large base64 image strings to parse smoothly
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    if (!customerName || !customerEmail || !customerPhone) {
      return res.status(400).json({ error: 'Missing customer identity markers (Name, Email, or Phone).' });
    }
    if (!shippingAddress || !shippingAddress.street || !shippingAddress.city) {
      return res.status(400).json({ error: 'Incomplete physical transit routing address layout.' });
    }
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ error: 'Your manifest is empty. Cannot process empty cart states.' });
    }

    const order = new Order({ customerName, customerEmail, customerPhone, shippingAddress, items, total });
    await order.save();
    res.status(201).json({ success: true, order });
  } catch (err) {
    res.status(500).json({ error: 'Internal server error processing checkout metrics.' });
  }
});
...
// Serve frontend assets cleanly from compiled Vite build directory
